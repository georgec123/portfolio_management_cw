...
def Haircut_SR(sm_fre, num_obs, SR, ind_an, ind_aut, rho, num_test, RHO):
    ...
    sm_fre_fre_out_dict = {1: 'Daily', 2: 'Weekly', 3: 'Monthly', 4: 'Quarterly', 5: 'Annual'}
    ...
    fn_sr_annual = lambda rho, sr, fre: sr*(1 + (2*rho/(1-rho)) * (1 - ((1-rho**(fre))/(fre*(1-rho)))))**(-0.5)
    ...
    if ind_an == 1 and ind_aut == 0:
        ...
        sr_annual = fn_sr_annual(rho, SR, sm_fre_days_dict[sm_fre]) * math.sqrt(sm_fre_days_dict[sm_fre])
